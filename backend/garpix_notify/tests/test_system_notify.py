...
from .utils.generate_data import (
    generate_users,
    generate_category,
    generate_system_notify_data,
    generate_system_template_data,
...
class TestSystemNotify(CommonTestClass):
    ...
    def setup(self):
        ...
        self.data_compiled_system_1 = generate_compiled_system(self.some_data_dict, self.PASS_TEST_EVENT, self.user_1)[
            0
        ]
    ...
    def test_system_notify_positive(self, setup):
        ...
        SystemNotify.send(self.some_data_dict, user=self.user_1, event=self.PASS_TEST_EVENT)
    ...
    def test_system_notify_user_list(self, setup):
        ...
        for notify in notify_qs:
            self.data_compiled_system_2 = generate_compiled_system(
                self.some_data_dict, self.PASS_TEST_EVENT_1, notify.user
            )[0]
