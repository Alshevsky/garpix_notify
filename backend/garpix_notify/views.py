...
def send_webhook(request):
    ...
    viber = Api(BotConfiguration(name=config.viber_bot_name, avatar="", auth_token=config.viber_api_key))
    current_host = request.META["HTTP_HOST"]
    print(reverse("viber_check_webhook"), "reverse(viber_check_webhook)")
    viber.set_webhook(
        url=f"https://{current_host}" + reverse("viber_check_webhook"),
...
def conversation(response):
    ...
    if response["subscribed"] == "false":
        ...
        viber.send_messages(
            to=id_user, messages=[TextMessage(text=f"Привет, {user_name}, {config.viber_welcome_text}")]
        )
...
def add_viber_user(response):
    ...
    if viber_user is not None and viber_user.viber_secret_key == secret_key:
        ...
        viber_user.save()
        viber.send_messages(
            to=viber_user_id, messages=[TextMessage(text=f"{viber_user_name}, {config.viber_success_added_text}")]
        )
    else:
        viber.send_messages(
            to=viber_user_id, messages=[TextMessage(text=f"{viber_user_name}, {config.viber_failed_added_text}")]
        )
...
def subscribed_event(response):
    ...
    id_user = response["user"]["id"]
    user_name = response["user"]["name"]
    viber.send_messages(to=id_user, messages=[TextMessage(text=f"{user_name},  {config.viber_text_for_new_sub}")])
