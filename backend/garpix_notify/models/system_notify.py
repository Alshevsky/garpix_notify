...
SystemNotifyMixin = import_string(
    getattr(settings, "GARPIX_SYSTEM_NOTIFY_MIXIN", "garpix_notify.mixins.notify_mixin.NotifyMixin")
)
...
class SystemNotify(SystemNotifyMixin, models.Model):
    ...
    def send(
        ...
        **kwargs,
        ...
        if event and templates is None:
            templates = list(
                NotifyTemplate.objects.filter(event=event, is_active=True, type=TYPE.SYSTEM).values_list(
                    "id", flat=True
                )
            )
        ...
        if templates:
            templates_instances = (
                NotifyTemplate.objects.select_related("user")
            ...
            if templates_instances.exists():
                for template in templates_instances:
                    ...
                    if template.user_lists.exists():
                        ...
                        system_notify_user_list.extend([user for user in notify_users_list if user])
    ...
    def send_notification(self):
        try:
            ...
            async_to_sync(get_channel_layer().group_send)(group_name, notify_dict)
