...
class NotifyConfig(SingletonModel):
    ...
    sms_url_type = models.IntegerField(
        default=SMS_URL.SMSRU_ID, choices=SMS_URL.TYPES, verbose_name="URL СМС провайдера"
    )
    sms_api_id = models.CharField(
        default="1234567890", blank=True, max_length=255, verbose_name="API ID СМС провайдера"
    )
    sms_login = models.CharField(
        default="", blank=True, max_length=255, verbose_name="Логин пользователя СМС провайдера"
    )
    sms_password = models.CharField(
        default="", blank=True, max_length=255, verbose_name="Пароль для api СМС провайдера"
    )
    sms_from = models.CharField(
        default="", blank=True, max_length=255, verbose_name="Отправитель СМС", help_text="Например, Garpix"
    )
    call_url_type = models.IntegerField(
        default=CALL_URL.SMSRU_CALL_API_ID, choices=CALL_URL.TYPES, verbose_name="URL звонка провайдера"
    )
    call_api_id = models.CharField(
        default="1234567890", blank=True, max_length=255, verbose_name="API ID оператора связи"
    )
    call_login = models.CharField(
        default="", blank=True, max_length=255, verbose_name="Логин/Индетификатор сервиса оператора связи"
    )
    call_password = models.CharField(
        default="", blank=True, max_length=255, verbose_name="Пароль/Секретный ключ оператора связи"
    )
    ...
    telegram_bot_name = models.CharField(
        default="", blank=True, help_text="Например, MySuperBot", max_length=255, verbose_name="Telegram Имя бота"
    )
    telegram_welcome_text = models.TextField(
        blank=True,
        default="Добрый день! Здесь вы можете получать уведомления от нашего сайта",
        verbose_name="Telegram - Приветственный текст бота",
    )
    telegram_help_text = models.TextField(
        blank=True,
    ...
    telegram_bad_command_text = models.TextField(
        blank=True, default="Неправильный формат команды", verbose_name="Telegram - Текст неправильной команды бота"
    )
    telegram_success_added_text = models.TextField(
        blank=True,
    ...
    telegram_failed_added_text = models.TextField(
        blank=True,
    ...
    telegram_parse_mode = models.CharField(
        default=PARSE_MODE_TELEGRAM.EMPTY,
        choices=PARSE_MODE_TELEGRAM.TYPES,
        verbose_name="Тип парсера телеграм сообщений",
        max_length=100,
        blank=True,
    )
    telegram_disable_notification = models.BooleanField(
        verbose_name="Пользователи получат уведомление без звука", default=False
    )
    telegram_disable_web_page_preview = models.BooleanField(
        verbose_name="Отключает предварительный просмотр ссылок в сообщениях", default=False
    )
    ...
    viber_api_key = models.CharField(
        default="000000000:AAAAAAAAAA-AAAAAAAA-_AAAAAAAAAAAAAA",
        blank=True,
        max_length=255,
        verbose_name="Viber API Key",
    )
    viber_bot_name = models.CharField(
        blank=True, max_length=255, verbose_name="Название viber бота", default="Viber bot"
    )
    whatsapp_sender = models.CharField(
        max_length=30, blank=True, default="", verbose_name="Телефон отправителя WhatsApp"
    )
    whatsapp_auth_token = models.CharField(
        default="XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX", blank=True, max_length=255, verbose_name="WhatsApp Auth Token"
    )
    whatsapp_account_sid = models.CharField(
        default="ACXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX", blank=True, max_length=255, verbose_name="WhatsApp Account SID"
    )
    ...
    viber_success_added_text = models.TextField(
        blank=True,
        default="Ваша учетная запись успешно привязана к боту. Вы будете получать уведомления!",
        verbose_name="Viber - Текст успешно добавлен код",
    )
    viber_failed_added_text = models.TextField(
        blank=True,
        default="Ошибка при привязке учетной записи. Пожалуйста, свяжитесь с техподдержкой",
        verbose_name="Viber - Текст провал, не добавлен код",
    )

    viber_text_for_new_sub = models.TextField(
        blank=True,
    ...
    viber_welcome_text = models.TextField(
        blank=True,
        default="для активации бота нужно отправить любое сообщения",
        verbose_name="Viber - Приветственный текст бота",
    )
    email_malling = models.IntegerField(
        default=EMAIL_MALLING.BCC,
        choices=EMAIL_MALLING.TYPES,
