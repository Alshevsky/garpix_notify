...
class SendData:
    ...
    def __get_call_url(self, key: int) -> Optional[str]:
        url_dict_call = {
            ...
            CALL_URL.SMSRU_CALL_ID: (
                f"{CALL_URL.SMSRU_CALL_URL}?login={self.CALL_LOGIN}&password={self.CALL_PASSWORD}"
                + "&phone={to}&json=1"
            ),
            CALL_URL.SMSCENTRE_ID: (
                f"{CALL_URL.SMSCENTRE_URL}?login={self.CALL_LOGIN}&psw={self.CALL_PASSWORD}"
                + "&phones={to}&mes=code&call=1&fmt=3"
            ),
            CALL_URL.UCALLER_ID: (
                f"{CALL_URL.UCALLER_URL}?key={self.CALL_PASSWORD}&service_id={self.CALL_LOGIN}" + "&phone={to}"
            ),
        }
    ...
    def __get_sms_url(self, key: int) -> Optional[str]:
        url_dict_sms = {
            ...
            SMS_URL.WEBSZK_ID: (
                f"{SMS_URL.WEBSZK_URL}?login={self.SMS_LOGIN}&pass={self.SMS_PASSWORD}&sourceAddress={self.SMS_FROM}"
                + "&destinationAddress={to}&data={text}"
            ),
            SMS_URL.IQSMS_ID: (
                f"{SMS_URL.IQSMS_URL}?login={self.SMS_LOGIN}&password={self.SMS_PASSWORD}" + "&phone={to}&text={text}"
            ),
            SMS_URL.INFOSMS_ID: (
                f"{SMS_URL.INFOSMS_URL}?login={self.SMS_LOGIN}&pwd={self.SMS_PASSWORD}&sender={self.SMS_FROM}"
                + "&phones={to}&message={text}"
            ),
            SMS_URL.SMSCENTRE_ID: (
                f"{SMS_URL.SMSCENTRE_URL}?login={self.SMS_LOGIN}&psw={self.SMS_PASSWORD}" + "&phones={to}&mes={text}"
            ),
            SMS_URL.SMS_SENDING_ID: (
                f"{SMS_URL.SMS_SENDING_URL}?login={self.SMS_LOGIN}&password={self.SMS_PASSWORD}" + "&txt={text}&to={to}"
            ),
            SMS_URL.SMS_PROSTO_ID: (
                f"{SMS_URL.SMS_PROSTO_URL}?login={self.SMS_LOGIN}&password={self.SMS_PASSWORD}&method=push_msg&format=json&sender_name={self.SMS_FROM}"
                + "text={text}&phone={to}&key={api_id}"
            ),
        }
